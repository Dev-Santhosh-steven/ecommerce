...
@section('title', 'Add LED Module')

@section('page-title', 'Add LED Module')

@section('content')
    <div class="max-w-5xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <h1 class="text-2xl font-bold">Create LED Module</h1>
        <p class="mt-1 text-sm text-slate-500">Active modules appear in the public LED Wall Calculator.</p>

        <form action="{{ route('admin.led-modules.store') }}" method="POST" class="mt-6">
            @csrf

            @include('admin.led-modules._form')

            <div class="mt-8 flex gap-3">
                <button type="submit" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white">Save Module</button>
